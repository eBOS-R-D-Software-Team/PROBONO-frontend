...
/** Fetch Dublin Solar rows between two ISO day timestamps (inclusive). */
export const fetchDubSolarData = createAsyncThunk(
  'dubSolar/fetch',
  async ({ startTime, endTime }, thunkAPI) => {
    try {
      ...
      return series.values.map((row) => {
        ...
        // copy numeric columns; skip strings like meter_point
...
const slice = createSlice({
  ...
  extraReducers: (b) => {
    b
      .addCase(fetchDubSolarData.pending,   (s)   => { s.loading = true;  s.error = null; })
      .addCase(fetchDubSolarData.fulfilled, (s,a) => { s.loading = false; s.data = a.payload; })
      .addCase(fetchDubSolarData.rejected,  (s,a) => { s.loading = false; s.error = a.payload; });
  },
});
